...
// inlineRenderer owns every row Cy draws. Keeping one source-backed buffer is
...
func (r *inlineRenderer) RenderFrame(frame inlineFrame, width, height int) error {
	...
	if !r.started {
		if err := r.write(ansi.SetModeBracketedPaste + ansi.SetModifyOtherKeys2 + ansi.KittyKeyboard(1, 1) + ansi.ResetModeTextCursorEnable); err != nil {
			return err
		}
		r.started = true
	}
