...
func TestFileChangeMetaRoundTripsThroughGenericJSON(t *testing.T) {
	original := buildFileChangeMeta("note.txt", "edited", []byte("before\n"), []byte("after\n"))
	generic := map[string]any{
		"type":        original.Type,
		"path":        original.Path,
		"operation":   original.Operation,
		"additions":   float64(original.Additions),
		"deletions":   float64(original.Deletions),
		"hunks":       original.Hunks,
		"total_hunks": float64(original.TotalHunks),
	}
	decoded, ok := fileChangeMetaFrom(generic)
	if !ok || decoded.Path != original.Path || decoded.Additions != 1 || decoded.Deletions != 1 || len(decoded.Hunks) != 1 {
