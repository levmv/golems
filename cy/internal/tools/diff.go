...
const MaxDiffDisplayLines = maxDiffDisplayLines
...
type FileChangeMeta = fileChangeMeta
type FileDiffHunk = fileDiffHunk
type FileDiffLine = fileDiffLine
...
func fileChangeMetaFrom(value any) (fileChangeMeta, bool) {
	...
	if typed, ok := value.(fileChangeMeta); ok {
		return typed, typed.Type == fileChangeMetaType
	}
	if typed, ok := value.(*fileChangeMeta); ok && typed != nil {
		return *typed, typed.Type == fileChangeMetaType
	}
	...
	if err := json.Unmarshal(raw, &decoded); err != nil || decoded.Type != fileChangeMetaType {
		return fileChangeMeta{}, false
	}
	return decoded, true
}
...
func FileChangeMetaFrom(value any) (FileChangeMeta, bool) {
	return fileChangeMetaFrom(value)
}
